...
// //         // --- SAFE FILE PROCESSING (Wont Crash on Error) ---
// //         const processFile = async (file, suffix, index = '') => {
// //             if (!file) return null;
...
// //             try {
// //                 console.log(`🚀 Attempting Upload: ${finalName}`);
// //                 // Try Uploading
// //                 const nasPath = await uploadToSynology(file.path, metadata, finalName);
                
// //                 // If NAS returns a path, use it. If null, use filename prepended with FAILED
// //                 const resultPath = nasPath || ("FAILED_UPLOAD_" + finalName);
...
// //             } catch (err) {
// //                 console.error(`⚠️ Upload Error (${finalName}):`, err.message);
// //                 // Return a placeholder string so DB save still works
// //                 return "ERROR_UPLOAD_" + finalName; 
// //             }
// //         };
...
// //             if (req.files['photos']) {
// //                 for (let i = 0; i < req.files['photos'].length; i++) {
// //                     const p = await processFile(req.files['photos'][i], 'photo', i > 0 ? `_${i}` : '');
// //                     if (p) photoPaths.push(p);
// //                 }
// //             }
// //             if (req.files['videos']) {
// //                 for (let i = 0; i < req.files['videos'].length; i++) {
// //                     const p = await processFile(req.files['videos'][i], i === 0 ? 'video' : 'gopro');
...
// //     try {
// //         const result = await db.query("SELECT * FROM surveys ORDER BY created_at DESC");
// //         const surveys = result.rows.map(s => ({...s, mediaFiles: []})); // Simplified for stability
// //         res.json({ surveys });
// //     } catch (err) { res.status(500).json({ error: err.message }); }
// // };
...
// // export const readFile = async (req, res) => { res.status(404).send("Feature temporarily disabled"); };
...
//         // --- ROBUST FILE PROCESSING (PREVENTS 500 ERROR) ---
//         const processFile = async (file, suffix, index = '') => {
//             if (!file) return null;
...
//                 // If NAS login failed but didn't throw error
//                 return "FAILED_UPLOAD_" + finalName; 

//             } catch (err) {
//                 // 4. CATCH ERROR (Do not crash the server!)
//                 console.error(`⚠️ Upload Error for ${finalName}:`, err.message);
                
//                 // Cleanup local file even if error
//                 if (fs.existsSync(file.path)) fs.unlinkSync(file.path);

//                 // Return a text string so DB still saves
//                 return "ERROR_" + finalName; 
//             }
//         };
...
//         const result = await db.query("SELECT * FROM surveys ORDER BY created_at DESC");
//         const surveys = result.rows.map(s => {
//             // Basic media handling for stability
//             return { ...s, mediaFiles: [] }; 
//         });
//         res.json({ surveys });
//     } catch (err) { res.status(500).json({ error: err.message }); }
...
// export const readFile = async (req, res) => { res.status(404).send("Feature disabled"); };
...
export const createSurvey = async (req, res) => {
    try {
        ...
        const processFile = async (file, suffix, index = '') => {
            ...
            try {
                // Try Uploading with 4s timeout (handled in service)
                const nasPath = await uploadToSynology(file.path, metadata, finalName);
                
                // Cleanup local file immediately to free memory
                if (fs.existsSync(file.path)) fs.unlinkSync(file.path);
                
                // If upload succeeded, return path. If null (timeout), return fallback name.
                if (nasPath) return nasPath;
                else return "UPLOAD_FAILED_" + finalName;

            } catch (err) {
                console.error(`Local Error processing ${finalName}:`, err.message);
                if (fs.existsSync(file.path)) fs.unlinkSync(file.path);
                return "ERROR_" + finalName; 
            }
        };
        ...
        const result = await db.query(query, values);
        console.log("✅ DB Success! ID:", result.rows[0].id);
        res.json({ success: true, survey: result.rows[0] });
...
export const readFile = async (req, res) => { res.status(404).send("Disabled"); };