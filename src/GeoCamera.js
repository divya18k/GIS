...
// //       mapUpdateInterval.current = setInterval(() => {
// //           if(mapContainerRef.current) {
// //               html2canvas(mapContainerRef.current, { useCORS: true, scale: 0.5, logging: false, backgroundColor: null }).then(canvas => {
...
// //     recorder.ondataavailable = e => { if (e.data.size > 0) chunksRef.current.push(e.data); };
// //     recorder.onstop = () => {
// //         // --- FIX: USE 'video/mp4' TYPE HERE TO TRICK ANDROID ---
...
//       try {
//         const constraints = {
//             // Selfie = Front Camera, Others = Rear
...
//   // 3. Map Snapshotter (Skip for Selfie)
//   useEffect(() => {
//       if (mode === 'selfie') return; // Don't process map for selfie
...
//     // --- ONLY DRAW OVERLAYS IF NOT SELFIE ---
//     if (mode !== 'selfie') {
//         const grad = ctx.createLinearGradient(0, 0, 0, 150);
//         grad.addColorStop(0, "rgba(0,0,0,0.8)");
//         grad.addColorStop(1, "transparent");
...
//   // 5. Capture Photo
//   const takePhoto = () => {
//     // --- IF SELFIE: CLEAN CAPTURE ---
//     if (mode === 'selfie') {
//         const canvas = canvasRef.current;
//         drawToCanvas(); // Ensure frame is drawn
//         // Use standard toBlob for simple clean image
//         canvas.toBlob(blob => {
//             const url = URL.createObjectURL(blob);
//             onCapture(url, blob, coordsRef.current);
//             onClose();
...
//   // 6. Record Video (Only allowed if NOT selfie mode based on logic, but keeping function for safety)
//   const startRecording = () => {
//     setIsRecording(true);
//     drawToCanvas(); 
//     const stream = canvasRef.current.captureStream(30); 
...
export default function GeoCamera({ mode, onCapture, onClose, metaData }) {
  ...
  // 3. Map Snapshotter (Runs in background)
  useEffect(() => {
  ...
  // 5. Capture Photo (OPTIMIZED FOR SPEED)
  const takePhoto = () => {
    // Determine canvas based on mode
    const canvas = canvasRef.current;
    ...
    setTimeout(() => {
        if(canvas) {
            ...
            // If Selfie, no EXIF needed, just return
            if (mode === 'selfie') {
                canvas.toBlob(blob => {
                    const url = URL.createObjectURL(blob);
                    onCapture(url, blob, coordsRef.current);
