...
//     const [searchDist, setSearchDist] = useState('');
//     const [searchBlock, setSearchBlock] = useState('');
//     const [searchGeneric, setSearchGeneric] = useState(''); // For manual typing search
//     const [searchDateFrom, setSearchDateFrom] = useState('');
//     const [searchDateTo, setSearchDateTo] = useState('');
//     const [filterStart, setFilterStart] = useState('');
//     const [filterEnd, setFilterEnd] = useState('');
...
//         // 1. Dropdown/Manual District Filter
//         if (searchDist) {
//             filtered = filtered.filter(s => s.district && s.district.toLowerCase().includes(searchDist.toLowerCase()));
//         }
//         // 2. Dropdown/Manual Block Filter
//         if (searchBlock) {
//             filtered = filtered.filter(s => s.block && s.block.toLowerCase().includes(searchBlock.toLowerCase()));
//         }
//         // 3. Generic Text Search (Route, Filename)
//         if (searchGeneric) {
...
//             {showSurveyTable && (
//                 <ModalWrapper title="Survey Database" onClose={() => setShowSurveyTable(false)}>
//                     {/* NEW: DATALIST & INPUTS FOR MANUAL TYPING + DROPDOWN */}
//                     <div style={styles.filterBox}>
//                         {/* General Search */}
...
const Dashboard = ({ user, role, onLogout, logAction }) => {
    ...
    const styles = {
        ...
        // HEADER: Exactly like Desktop view
        header: { 
            padding: '10px 20px', 
            background: '#1a237e', 
            color: 'white', 
            display: 'flex', 
            flexWrap: 'wrap', // Allow wrapping only if screen is tiny
        ...
        select: { padding: '8px 12px', borderRadius: '4px', minWidth: '120px', border:'1px solid #ccc', background:'white', fontSize:'13px', cursor:'pointer' },
    ...
    return (
        <div style={styles.container}>
            {/* Header Logic: Keeps everything side-by-side (Desktop Look) but wraps if needed */}
            <div style={styles.header}>
