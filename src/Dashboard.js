...
//     const styles = {
//         container: { display: 'flex', flexDirection: 'column', height: '100vh', fontFamily: 'Arial, sans-serif', overflow:'hidden' },
        
//         // HEADER: No Wrapping (Scrolls horizontally on mobile)
//         header: { 
//             padding: '10px 15px', 
//             background: '#1a237e', 
//             color: 'white', 
...
//             overflowX: 'auto'     // ENABLE SCROLLING IF TOO NARROW
//         },
...
const Dashboard = ({ user, role, onLogout, logAction }) => {
    ...
    const refreshData = useCallback(async () => {
        try {
            ...
            // Only update logs/users here, don't touch dropdowns
            setLogs(JSON.parse(localStorage.getItem('bsnl_logs')) || []);
            setUserStatuses(JSON.parse(localStorage.getItem('bsnl_users')) || []);
    ...
    // FIXED: Refresh interval only updates data, doesn't reset component state
    useEffect(() => {
        refreshData();
        const interval = setInterval(refreshData, 5000); 
        return () => clearInterval(interval);
        ...
        <div style={styles.container}>
            <div style={styles.header}>
                <div style={{display:'flex', alignItems:'center'}}>
                    ...
                    <div style={{marginLeft:'30px', display:'flex', gap:'10px'}}>
                        <select style={styles.select} value={selectedDistrict} onChange={e=>setSelectedDistrict(e.target.value)}><option>District</option>{visibleDistricts.map(d=><option key={d}>{d}</option>)}</select>
                        <select style={styles.select} value={selectedBlock} onChange={e=>setSelectedBlock(e.target.value)}><option>Block</option>{blockOptions.map(b=><option key={b}>{b}</option>)}</select>
                        <select style={styles.select} value={selectedSpan} onChange={e=>setSelectedSpan(e.target.value)}><option>Span</option>{spanOptions.map(s=><option key={s}>{s}</option>)}</select>
                        <select style={styles.select} value={selectedRing} onChange={e=>setSelectedRing(e.target.value)}><option>Ring (Opt)</option>{ringOptions.map(r=><option key={r}>{r}</option>)}</select>
                    </div>
                </div>
